...
function HienThiDIV(){
    if(search.style.height === "0px"){
        ...
        var KQinput = document.querySelector("#KQ_Search")
        KQinput.style.display = "none"
    }
...
function SearchProduct(event){
    ...
    // console.log(inputvalue)
    var KQinput = document.querySelector("#KQ_Search")
    KQinput.style.display = "block"

    var tbody = document.querySelector("#content")
    var content=""
    listProduct.forEach(function(value, index){

        if(listProduct[index].TenSanPham.toLowerCase().startsWith(inputvalue.toLowerCase())){
            //startsWith để kiểm tra xem chuỗi value.name
            // có bắt đầu bằng giá trị của searchTerm hay không.
            content +=`
                        <tr onclick="SelectProduct(${value.MaSanPham})">  
                           
                                <td style="width:40px;padding:2px;"><img src="/anh/${value.AnhDaiDien}" alt="" style="width: 100%;"></td>
                                <td style="padding: 5px;width: 205px;">${value.TenSanPham}</td>
                                <td style="padding: 5px;">${value.Gia}đ</td>
...
function SelectProduct(productId){
    ...
    var url = '/user/detail?id=' + productId;
